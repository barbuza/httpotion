  ...
  defmacro __using__(_) do
    quote do
      ...
      def process_headers(headers) do
        :orddict.from_list(Enum.map headers, fn ({k, v}) -> { binary_to_atom(iolist_to_binary(k)), iolist_to_binary(v) } end)
      end
      ...
      def request(method, url, body // "", headers // [], options // []) do
        url = to_char_list process_url(iolist_to_binary(url))
        ...
        case :ibrowse.send_req(url, headers, method, body, ib_options, timeout) do
          ...
          {:error, {:conn_failed, {:error, reason}}} ->
            raise HTTPotion.HTTPError[message: iolist_to_binary(reason)]
          {:error, :conn_failed} ->
